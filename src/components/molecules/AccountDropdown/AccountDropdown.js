...
export const AccountDropdown = () => {
  ...
  const filterSmallBalances = (currency) => {
    const balance = wallet[currency].vlue;
    console.log(balance);
    const usdBalance = coinEstimator(currency) * wallet[currency].value;
                ...
                .map((ticker, key) => {
                  return (
                    <CurrencyListItem key={key}>
                      ...
                      <div>
                        <strong>
                          {(wallet[ticker].value / 100000000000000000).toFixed(3)} {ticker}
                        </strong>
                        <small>
                          $
                          {formatUSD(
                            coinEstimator(ticker) * wallet[ticker].value / 100000000000000000
                          )}
                        </small>
                      </div>
                    </CurrencyListItem>
